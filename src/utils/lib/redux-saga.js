...
konst(false);
  ...
  let identity = function identity(v) {
    return v;
  };
...
autoInc();
  ...
  let kThrow = function kThrow(err) {
    throw err;
  };

  let kReturn = function kReturn(value) {
    return {
      value: value,
      done: true
    };
 ...
 **/
  ...
  let internalErr = function internalErr(err) {
    return new Error(`\n  redux-saga: Error checking hooks detected an inconsistent state. This is likely a bug\n  in redux-saga code and not yours. Thanks for reporting this in the project's github repo.\n  Error: ${err}\n`);
  };
  let createSetContextWarning = function createSetContextWarning(ctx, props) {
    return `${ctx ? `${ctx}.` : ''}setContext(props): argument ${props} is not a plain object`;
  };
  let wrapSagaDispatch = function wrapSagaDispatch(dispatch) {
  ...
  let cloneableGenerator = function cloneableGenerator(generatorFunc) {
    return function() {
      ...
      return {
        ...
        clone: function clone() {
          let clonedGen = cloneableGenerator(generatorFunc)(...args);
          history.forEach((arg) => {
  ...
  let BUFFER_OVERFLOW = 'Channel\'s Buffer overflow!';
  let ON_OVERFLOW_THROW = 1;
  let ON_OVERFLOW_DROP = 2;
  let ON_OVERFLOW_SLIDE = 3;
  let ON_OVERFLOW_EXPAND = 4;
  let zeroBuffer = {
    isEmpty: kTrue,
    put: noop,
    take: noop
  ...
  function ringBuffer(limit, overflowAction) {
    ...
    let push = function push(it) {
      arr[pushIndex] = it;
      pushIndex = (pushIndex + 1) % limit;
      length++;
    };

    let take = function take() {
      if (length != 0) {
        let it = arr[popIndex];
        arr[popIndex] = null;
        length--;
  ...
  let fixed = function fixed(limit) {
    return ringBuffer(limit, ON_OVERFLOW_THROW);
  };
  let dropping = function dropping(limit) {
    return ringBuffer(limit, ON_OVERFLOW_DROP);
  };
  let sliding = function sliding(limit) {
    return ringBuffer(limit, ON_OVERFLOW_SLIDE);
  };
  let expanding = function expanding(initialSize) {
    return ringBuffer(initialSize, ON_OVERFLOW_EXPAND);
  };



  let buffers = Object.freeze({
    none: none,
    fixed: fixed,
    dropping: dropping,
    sliding: sliding,
...
**/
  ...
  let array$1 = function array$$1(patterns) {
  ...
  let predicate = function predicate(_predicate) {
    return function(input) {
      return _predicate(input);
    };
  };
  let string = function string(pattern) {
    return function(input) {
      return input.type === String(pattern);
    };
  };
  let symbol = function symbol(pattern) {
    return function(input) {
  ...
  // prettier-ignore
    let matcherCreator = pattern === '*' ? wildcard : is.string(pattern) ? string : is.array(pattern) ? array$1 : is.stringableFunc(pattern) ? string : is.func(pattern) ? predicate : is.symbol(pattern) ? symbol : null;
  ...
  let isEnd = function isEnd(a) {
    return a && a.type === CHANNEL_END_TYPE;
  };
  let INVALID_BUFFER = 'invalid buffer passed to channel factory function';
  ...
  function multicastChannel() {
    ...
    let ensureCanMutateNextTakers = function ensureCanMutateNextTakers() {
    ...
    let close = function close() {
      closed = true;
      let takers = currentTakers = nextTakers;
...
/* global window */
  ...
  let result = symbolObservablePonyfill(root$2);
...
*/
  ...
  function fsmIterator(fsm, q0, name) {
    ...
    return makeIterator(next, (error) => {
  ...
  function takeEvery$1(patternOrChannel, worker) {
    ...
    let yFork = function yFork(ac) {
      return {
        done: false,
        value: fork(...[worker].concat(args, [ac]))
      };
    };
  ...
  function takeLatest$1(patternOrChannel, worker) {
    ...
    let yCancel = function yCancel(task) {
      return {
        done: false,
        value: cancel(task)
      };
    ...
    let setTask = function setTask(t) {
      return task = t;
    };

    let setAction = function setAction(ac) {
      return action = ac;
    };
  ...
  function throttle$1(delayLength, pattern, worker) {
    ...
    let action, channel$$1;
    let yActionChannel = {
      done: false,
      value: actionChannel(pattern, sliding(1))
    };

    let yTake = function yTake() {
      return {
        done: false,
    ...
    let setChannel = function setChannel(ch) {
      return channel$$1 = ch;
    };
  ...
  let CANCEL$1 = 'CANCEL';
  let SELECT = 'SELECT';
  let ACTION_CHANNEL = 'ACTION_CHANNEL';
  let CANCELLED = 'CANCELLED';
  let FLUSH = 'FLUSH';
  let GET_CONTEXT = 'GET_CONTEXT';
  let SET_CONTEXT = 'SET_CONTEXT';
  let TEST_HINT = '\n(HINT: if you are getting this errors in tests, consider using createMockTask from redux-saga/utils)';

  let effect = function effect(type, payload) {
    let _ref;

    return _ref = {}, _ref[IO] = true, _ref[type] = payload, _ref;
  };

  let detach = function detach(eff) {
    {
  ...
  let takeMaybe = function takeMaybe() {
    let eff = take(...arguments);
    eff[TAKE].maybe = true;
    return eff;
  };
  ...
  let putResolve = function putResolve() {
    let eff = put(...arguments);
  ...
  function spawn(fn) {
    ...
    return detach(fork(...[fn].concat(args)));
  }
  function join() {
...
**/
  ...
  function takeEvery(patternOrChannel, worker) {
    ...
    return fork(...[takeEvery$1, patternOrChannel, worker].concat(args));
  }
  function takeLatest(patternOrChannel, worker) {
    ...
    return fork(...[takeLatest$1, patternOrChannel, worker].concat(args));
  }
  function throttle(ms, pattern, worker) {
    ...
    return fork(...[throttle$1, ms, pattern, worker].concat(args));
  }
  var delay$1 =
/* #__PURE__*/
call.bind(null, delay);

  let createAsEffectType = function createAsEffectType(type) {
...
**/
  ...
  function proc(iterator, stdChannel$$1, dispatch, getState, parentContext, options, parentEffectId, name, cont) {
    ...
    let log$$1 = logger || log;
    let taskContext = Object.create(parentContext);
  /**
  ...
  **/
    ...
    let task = newTask(parentEffectId, name, iterator, cont);
    let mainTask = {
      name: name,
      cancel: cancelMain,
      isRunning: true
    };
    let taskQueue = forkQueue(name, mainTask, end);
  /**
  ...
  **/
    ...
    **/
    ...
    function resolvePromise(promise, cb) {
      let cancelPromise = promise[CANCEL];
    ...
    function runTakeEffect(_ref2, cb) {
      let _ref2$channel = _ref2.channel,
          channel$$1 = _ref2$channel === void 0 ? stdChannel$$1 : _ref2$channel,
          pattern = _ref2.pattern,
          maybe = _ref2.maybe;

      let takeCb = function takeCb(input) {
    ...
    **/
    ...
    function runCallEffect(_ref4, effectId, cb) {
      let context = _ref4.context,
          fn = _ref4.fn,
          args = _ref4.args;
      let result; // catch synchronous failures; see #152
    ...
    // catch synchronous failures; see #152
      try {
        let cpsCb = function cpsCb(err, res) {
    ...
    function runForkEffect(_ref6, effectId, cb) {
      ...
      let taskIterator = createTaskIterator({
        context: context,
        fn: fn,
        args: args
      });
      ...
      try {
        ...
        let _task = proc(taskIterator, stdChannel$$1, dispatch, getState, taskContext, options, effectId, fn.name, detached ? null : noop);
    ...
    function runAllEffect(effects, effectId, cb) {
      ...
      keys.forEach((key) => {
        let chCbAtKey = function chCbAtKey(res, isErr) {
          if (completed) {
            return;
          }
      ...
      keys.forEach((key) => {
        let chCbAtKey = function chCbAtKey(res, isErr) {
          ...
          // Race Auto cancellation
            ...
            cb.cancel();
            completed = true;
            let response = (_response = {}, _response[key] = res, _response);
    ...
    function runSelectEffect(_ref7, cb) {
      let selector = _ref7.selector,
          args = _ref7.args;

      try {
        let state = selector(...[getState()].concat(args));
        cb(state);
      } catch (error) {
        cb(error, true);
      }
    ...
    function runChannelEffect(_ref8, cb) {
      let pattern = _ref8.pattern,
          buffer = _ref8.buffer;
    // TODO: rethink how END is handled
      let chan = channel(buffer);
      let match = matcher(pattern);

      let taker = function taker(action) {
        if (!isEnd(action)) {
  ...
  let RUN_SAGA_SIGNATURE = 'runSaga(options, saga, ...args)';
  let NON_GENERATOR_ERR = `${RUN_SAGA_SIGNATURE}: saga argument must be a Generator function!`;
  function runSaga(options, saga) {
    ...
    let _options$channel = options.channel,
    ...
    if (('development' === 'development' || 'development' === 'test') && is.notUndef(effectMiddlewares)) {
      let MIDDLEWARE_TYPE_ERROR = 'effectMiddlewares must be an array of functions';
    ...
    let middleware = effectMiddlewares && compose(...effectMiddlewares);
    let task = proc(iterator, channel$$1, wrapSagaDispatch(dispatch), getState, context, {
  ...
  function sagaMiddlewareFactory(_ref) {
    ...
    let sagaMonitor = options.sagaMonitor,
        logger = options.logger,
        onError = options.onError,
        effectMiddlewares = options.effectMiddlewares;
    ...
    function sagaMiddleware(_ref3) {
      ...
      return function(next) {
        return function(action) {
          ...
          let result = next(action); // hit reducers
